...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set([panel, ...preLayers], { xPercent: offscreen, willChange: 'transform' });
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (reducedMotion) {
      ...
      if (numberEls.length) gsap.set(numberEls, { ['--sm-num-opacity' as any]: 1 });
    ...
    layerStates.forEach((ls, i) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.35, ease: 'power4.out' }, i * 0.05);
    });
    ...
    if (itemEls.length) {
      ...
      tl.to(
        itemEls,
        { yPercent: 0, rotate: 0, duration: 0.6, ease: 'power4.out', stagger: { each: 0.06, from: 'start' } },
        itemsStart
      );
    ...
    if (socialTitle || socialLinks.length) {
      ...
      if (socialTitle) tl.to(socialTitle, { opacity: 1, duration: 0.35, ease: 'power2.out' }, socialsStart);
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    const cycles = reducedMotion ? 1 : 3;
    ...
    textCycleAnimRef.current = gsap.to(inner, {
      yPercent: -finalShift,
      duration: reducedMotion ? 0.2 : 0.5 + lineCount * 0.07,
      ease: 'power4.out'
    });
  }, [reducedMotion]);
